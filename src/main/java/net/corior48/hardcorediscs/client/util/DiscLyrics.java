package net.corior48.hardcorediscs.client.util;
        ...
        for (Map.Entry<ResourceLocation, Resource> resourceEntry : resources.entrySet()) {
            ...
            try (InputStreamReader reader = new InputStreamReader(
                ...
                if (entry == null || entry.item() == null || entry.item().isBlank()) {
                    HardcoreDiscs.LOGGER.warn("Lyrics file {} has no disc id", fileId);
                    continue;
                }
                ...
                if (discId == null) {
                    HardcoreDiscs.LOGGER.warn("Lyrics file {} has invalid disc id {}", fileId, entry.item());
                    continue;
                }
                ...
                if (item == null || item == net.minecraft.world.item.Items.AIR) {
                    HardcoreDiscs.LOGGER.warn("Lyrics file {} points to missing item {}", fileId, discId);
                    continue;
                }

                LYRICS.put(item, entry);
            } catch (Exception e) {
                HardcoreDiscs.LOGGER.error("Failed to load lyrics file {}", fileId, e);
            }
        }

        loaded = true;
        HardcoreDiscs.LOGGER.info("Loaded {} lyric file(s)", LYRICS.size());
    }
